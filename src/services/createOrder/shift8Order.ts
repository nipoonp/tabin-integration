...
const getShift8Items = (mappingData, tabinOrder: IGET_RESTAURANT_ORDER_FRAGMENT) => {
  ...
  tabinOrder.products.forEach((product) => {
    ...
    const productTotalExTaxRounded = convertCentsToDollarsReturnFloat(
      product.price - productTotalTax,
    );

    const shift8Item: SHIFT8_ORDER_ITEM = {
      ItemID: sno,
      ItemPLU: parseInt(mappingData[product.id], 10),
    ...
    product.modifierGroups?.forEach((modifierGroup) => {
      modifierGroup.modifiers?.forEach((modifier) => {
        ...
        const modifierTotalExTaxRounded = convertCentsToDollarsReturnFloat(
          modifier.price - modifierTotalTax,
        );

        const shift8Mod: SHIFT8_ORDER_ITEM_MOD = {
          ModPLU: parseInt(mappingData[modifier.id], 10),
        ...
        modifier.productModifiers?.forEach((productModifier) => {
          ...
          productModifier.modifierGroups?.forEach((productModifierGroup) => {
            productModifierGroup.modifiers?.forEach((productModifierGroupModifier) => {
              const productModifierGroupModifierTotalRounded = convertCentsToDollarsReturnFloat(
                productModifierGroupModifier.price,
              );
              const productModifierGroupModifierTotalTax = calculateTaxAmount(
                productModifierGroupModifier.price,
              );
              const productModifierGroupModifierTotalTaxRounded = convertCentsToDollarsReturnFloat(
                productModifierGroupModifierTotalTax,
              );
              ...
              const shift8ItemMod: SHIFT8_ORDER_ITEM_MOD = {
                ModPLU: parseInt(mappingData[productModifierGroupModifier.id], 10),
              ...
              // If quantity is more then 1, add item 'quantity' number of times.
              for (let pmgmIndex = 0;
                pmgmIndex < productModifierGroupModifier.quantity;
                pmgmIndex += 1
              ) {
...
const convertShift8Order = (
  ...
  const tabinOrderTotalExTaxRounded = convertCentsToDollarsReturnFloat(
    tabinOrder.subTotal - tabinOrderTotalTax,
  );
  ...
  const shift8Sale: SHIFT8_CREATE_ORDER = {
    ...
    FulfillmentDate: tabinOrder.orderScheduledAt
      ? tabinOrder.orderScheduledAt : tabinOrder.placedAt,
    LocationNumber: shift8Credentials.storeLocationNumber,
...
export const getShift8AccessToken = async () => {
  ...
  const getSecrets = async () => {
    const shift8ApiConfig = await secretManager.getSecretValue({
      SecretId: apiConfigSecretId,
    }).promise();
    const shift8Config = JSON.parse(shift8ApiConfig.SecretString);

    const shift8ApiTokens = await secretManager.getSecretValue({
