...
const createOrder = async (
  ...
  doshiiOrder: IDOSHII_CREATE_ORDER,
) => {
  const token = sign(
    {
...
const convertDoshiiOrder = (
  ...
  const orderTaxes: IDOSHII_ORDER_TAX = {
    // posId: "",
    name: 'GST',
  ...
  const processModifierGroups = (
    modifierGroups: IGET_RESTAURANT_ORDER_MODIFIER_GROUP_FRAGMENT[],
  ): IDOSHII_ORDER_ITEM_OPTION[] => {
  ...
  tabinOrder.products.forEach((product) => {
    ...
    } else if (product.modifierGroups) {
      item.options = processModifierGroups(product.modifierGroups);
    }

    const itemTaxes: IDOSHII_ORDER_TAX = {
      // posId: "",
      name: 'GST',
      amount: String(taxRate * 100),
      type: 'percentage',
  ...
  //     name: tabinOrder.customerInformation?.firstName
  // ? tabinOrder.customerInformation.firstName : "Tabin",
  //     email: tabinOrder.customerInformation?.email
  // ? tabinOrder.customerInformation.email : "dev@tabin.co.nz",
  //     phone: tabinOrder.customerInformation?.phoneNumber
  // ? tabinOrder.customerInformation.phoneNumber : "+642102828894",
  //     address: {
  ...
  //     {
  //         amount: tabinOrder.payments && tabinOrder.payments.length > 0
  // ? tabinOrder.payments[0].amount : 0,
  //         reference: "",
  //         invoice: "",
  //         linkedTrxId: "11",
  //         method:
