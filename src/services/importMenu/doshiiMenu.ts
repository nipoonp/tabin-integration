...
const convertDoshiiMenu = async (doshiiMenu: IDOSHII_MENU) => {
  ...
  const processDoshiiProductOptions = (doshiiProduct?: IDOSHII_MENU_PRODUCT) => {
    doshiiProduct?.options?.forEach((doshiiOption: IDOSHII_MENU_OPTION, index: number) => {
      ...
      const isModifierGroupCreated = modifierGroups.some(
        (item) => item.modifierGroupId === doshiiOption.posId,
      );
      if (!isModifierGroupCreated) {
        const modifierGroup: IModifierGroup = {
          ...
          choiceMin: parseInt(doshiiOption.min ?? '0', 10),
          choiceMax: parseInt(doshiiOption.max ?? '0', 10),
      ...
      doshiiOption.variants?.forEach((doshiiVariant, index2: number) => {
        ...
        if (!isModifierCreated) {
          const modifier: IModifier = {
            ...
            price: parseInt(doshiiVariant.price ?? '0', 10),
          };
  ...
  const processIncludedItems = (
    ...
    const isProductCreated = products.some(
      (item) => item.productId === doshiiProductIncludedItem.posId,
    );
    ...
    if (!isProductCreated) {
      const product: IProduct = {
        ...
        price: parseInt(doshiiProductIncludedItem.unitPrice, 10),
    ...
    const modifierGroup: IModifierGroup = {
      ...
      choiceMin: parseInt(bundleMin || doshiiProductIncludedItem.quantity, 10),
      choiceMax: parseInt(bundleMax || doshiiProductIncludedItem.quantity, 10),
      choiceDuplicate: 1, // Modifier choiceDuplicate > 1 is not support in doshii
    };
  ...
  doshiiMenu.products?.forEach((doshiiProduct: IDOSHII_MENU_PRODUCT) => {
    ...
    if (!isProductCreated) {
      const product: IProduct = {
        ...
        price: parseInt(doshiiProduct.unitPrice ?? '0', 10),
        skuCode: doshiiProduct.productIds?.sku || '',
        soldOut: doshiiProduct.availability === 'unavailable',
      };
    ...
    doshiiProduct.includedItems?.forEach(
      (doshiiProductIncludedItem: IDOSHII_MENU_INCLUDED_ITEM, index) => {
        processIncludedItems(doshiiProduct, doshiiProductIncludedItem, index);
      },
    );

    doshiiProduct.bundledItems?.forEach(
      (doshiiProductBundledItem: IDOSHII_MENU_BUNDLED_ITEM, index) => {
        doshiiProductBundledItem.includedItems.forEach(
          (doshiiProductBundleIncludedItem: IDOSHII_MENU_INCLUDED_ITEM) => {
